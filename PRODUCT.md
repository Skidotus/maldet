...
- The plain-English findings summary is built (`llm_summary.py`), written by Ollama running locally at scan time and stored in `risk_scores.llm_summary`. It is an *explainer*, never a scorer: the Random Forest decides what's real and `calculate_risk()` decides how bad it is, both before the LLM sees anything, and the prompt states the scores as settled facts it must not re-rate. It receives the top ~15 finding groups rather than every finding — at corpus scale (~237k findings) per-finding LLM inference is hours-to-days of compute versus seconds for the Random Forest. Ollama is optional infrastructure: when it is absent, disabled, slow or unparsable, `app.py:build_findings_summary` (rule-based, counts plus the highest-priority finding) renders instead and a scan never fails because of it.
...
A 168-finding hand-reviewed evaluation set exists (`eval_sample.json`, built by `build_eval_sample.py`, scored by `evaluate_classifier.py`). Against it the classifier measures precision 0.52, recall 0.62, F1 0.56, versus a 0.28 precision baseline for treating every finding as real. These are the only ground-truth-backed numbers available; the higher figures printed by `train_classifier.py` are measured against its own weak labels and must not be quoted as accuracy. The hand labels were produced with AI-assisted first-pass review confirmed by the author, which should be disclosed wherever the numbers are cited.
